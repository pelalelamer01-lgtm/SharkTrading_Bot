...
# ================= 3. استقبال الأوامر بشكل قاطع وصحيح =================
async def check_telegram_commands():
    global BOT_RUNNING, LAST_UPDATE_ID
    try:
        # جلب التحديثات الجديدة فقط وبشكل فوري
        ...
        for update in updates:
            ...
            if update.message and str(update.message.chat_id) == CHAT_ID and update.message.text:
                ...
                # دعم كل الأشكال الممكنة لكلمة balance منعاً لأي لَبْس
                if "balance" in text:
                    try:
                        ...
                        await send_telegram_msg(f"⚠️ عذراً، فشل الاتصال بحساب باينانس لجلب الرصيد: {binance_err}")
                    ...
                    BOT_RUNNING = True
                    await send_telegram_msg("🚀 تم إعادة تشغيل الوحش! جاري فحص السيولة وفخاخ الحيتان وقنص الفرص الحكيمة.")
    except Exception as e:
...
async def main_bot_loop():
    ...
    # تهيئة أولية لتخطي الرسائل القديمة المعلقة تماماً
    try:
    ...
    await send_telegram_msg("⚔️ تم سحق المشكلة بنجاح وتأمين الاتصال اللحظي للأوامر!\n\n💵 اكتب الآن للبوت كلمة: balance\n(بأي شكل تحبه، بالشرطة أو بدونها، والوحش هيرد فوراً).")
    
    while True:
        try:
            # فحص الأوامر أولاً بشكل مستقل
            ...
            await asyncio.sleep(3) # فحص متوازن ومستقر جداً
