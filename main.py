...
BOT_RUNNING = True
# استخدام قيمة افتراضية مرنة لـ offset عشان يلقط الرسائل فوراً وبدون تعليق
LAST_UPDATE_ID = -1 
...
# ================= 3. استقبال أوامر التليجرام المحسنة والسريعة =================
async def check_telegram_commands():
    global BOT_RUNNING, LAST_UPDATE_ID
    try:
        # فحص جلب الرسائل مع التعامل الذكي مع الـ offset لمنع سقوط الأمر
        updates = await bot.get_updates(offset=LAST_UPDATE_ID, timeout=1, allowed_updates=["message"])
        
        for update in updates:
            LAST_UPDATE_ID = update.update_id + 1
            
            if update.message and str(update.message.chat_id) == CHAT_ID and update.message.text:
                # تنظيف النص تماماً من الفراغات وتحويله لـ 小写
                ...
                if "/balance" in text:
                    ...
                    BOT_RUNNING = False
                    await send_telegram_msg("🛑 تم إيقاف الوحش مؤقتاً.. لن يتم فتح صفقات جديدة حتى ترسل /start.")
                        
                elif "/start" in text:
                    BOT_RUNNING = True
                    await send_telegram_msg("🚀 تم إيقاظ الوحش بنجاح! جاري فحص مناطق السيولة وصيد فخاخ الحيتان فوراً.")
    except Exception as e:
        # طباعة الخطأ داخلياً لو الـ offset لسه بيتحمل لأول مرة
        print(f"Command Check Connection: {e}")
...
async def main_bot_loop():
    ...
    # تصفير الأوامر السابقة عند التشغيل عشان يستجيب للأمر الجديد فوراً
    try:
    ...
    await send_telegram_msg("🚀 تم تحديث نظام استقبال الأوامر بنجاح لضمان الاستقرار الفوري!\n💰 جرب الآن إرسال: /balance\n\n🤖 خوارزميات الاتفاق الكامل ومصيدة الحيتان تعمل بالخلفية بكفاءة 100%.")
    
    while True:
        try:
            # فحص دائم وسريع للأوامر
            ...
            await asyncio.sleep(2) # تقليل وقت الانتظار لـ 2 ثانية فقط عشان الاستجابة تكون طلقة!
