...
# ================= 2. إعدادات باينانس مع كسر الحظر الجغرافي للـ Testnet =================
...
# إنشاء الاتصال مع تجاوز البوابات الأمريكية عبر سيرفر بديل مباشر لـ Testnet
...
# تعديل روابط باينانس التجريبية يدوياً لضرب الحظر في مقتل (استخدام نقاط اتصال آسيوية وأوروبية بديلة)
exchange.urls['api']['test'] = 'https://api.binance.me/api' # تحويل المسار لنقطة تداول دولية مفتوحة
exchange.set_sandbox_mode(True) 

# إجبار ccxt على استخدام مسارات بديلة في الـ Testnet لتفادي الـ Block الأمريكي 451
    ...
    try:
        ...
        for update in updates:
            ...
            if update.message and str(update.message.chat_id) == CHAT_ID and update.message.text:
                ...
                if "balance" in text:
                    try:
                        # جلب الرصيد مع تحديث الأسواق لتحديث الاتصال الآمن
                        exchange.load_markets()
                        balance = exchange.fetch_balance()
                        usdt_balance = balance.get('USDT', {}).get('free', 0)
                        btc_balance = balance.get('BTC', {}).get('free', 0)
                        await send_telegram_msg(f"💰 الرصيد المخترق للحظر الجغرافي بنجاح:\n💵 {round(usdt_balance, 2)} USDT\n🪙 {round(btc_balance, 4)} BTC")
                    except Exception as binance_err:
                        await send_telegram_msg(f"⚠️ محاولة اتصال بديلة.. فشل بسبب: {binance_err}")
...
async def main_bot_loop():
    ...
    await send_telegram_msg("🛡️ تم تحويل مسار الطلبات بالكامل لنقطة اتصال دولية خارج أمريكا!\n\n💵 اكتب الآن للوحش: balance")
    ...
    while True:
        try:
            ...
            if BOT_RUNNING:
                ...
                except Exception as exchange_loop_err:
                    print(f"Market Fetch Error: {exchange_loop_err}")
